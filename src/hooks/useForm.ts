...
export interface FormInstance<Values = any> extends RcFormInstance<Values> {
  ...
  validateFieldsAndScroll: (nameList?: NamePath[]) => Promise<Store>;
  ...
  const wrapForm: FormInstance<Values> = React.useMemo(
    () =>
      form || {
        ...
        validateFieldsAndScroll: async (nameList: NamePath[], options: ScrollOptions = {}) => {
          let _error = null;
          let _values = null;
          await rcForm
            ...
            .catch(errorInfo => {
              wrapForm.scrollToField(errorInfo.errorFields[0].name, options);
